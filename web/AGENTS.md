...
-   **The web-add metadata fill (`src/lib/bookmarkMetadata.ts`, SPEC §5) never bumps `bookmarks.updated_at` either** — it fills `title`/`favicon_url` after a web add and seeds `note` from the scrape's summary, never overwrites a title the row already owns or an existing note (both guards re-checked in the UPDATE; a missing note alone never triggers a scrape), and never rejects (since m18 it runs ONLY fire-and-forget in `after()` — no request ever waits on it). Its Firecrawl call is injected, like the article pipeline's steps.
...
-   `/api/sync`, `/api/hello`, `/api/bookmarks/by-url` + `/api/tags` are Bearer-token endpoints and are excluded from the proxy matcher — keep them out of session logic. (`POST /api/highlights` and `POST /api/browse-events` are Bearer-authed too but stay matched: each shares its path with a session-authed sibling — `DELETE /api/highlights/:id`, `GET /api/browse-events` — and matched `/api/*` is never redirected.)
-   **Browse events (m19, SPEC §13) never touch the bookmarks table** — `applyBrowseEvents`/`listBrowseEvents` (`src/lib/browseEvents.ts`) are append-only telemetry over `browse_events` only: normalize URLs server-side, dedupe on `(user_id, client_event_id)` with DO NOTHING, never update or delete rows.
