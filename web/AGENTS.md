...
-   **The web-add metadata fill (`src/lib/bookmarkMetadata.ts`, SPEC §5) never bumps `bookmarks.updated_at` either** — it fills `title`/`favicon_url` after a web add, never overwrites a title the row already owns, and never rejects (it runs both inside the add request and in `after()`). Its Firecrawl call is injected, like the article pipeline's steps.
