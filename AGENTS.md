...
4. **Soft deletes only (bookmarks).** Never `DELETE` a bookmark row; set/clear `archived_at`. Highlights are hard-delete by design (SPEC §3). Browse events are hard-delete by design too, but ONLY through the m23 retention prune, and ONLY for weeks whose `attention_reports` card already exists — generate the card, verify it, then prune (SPEC §13 "Retention and report cards"). Nothing else may delete a `browse_events` row.
