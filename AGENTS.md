...
-   Vitest coverage is required for: URL normalization, upsert/bump/unarchive semantics, folder-tag derivation (`folderTags`) incl. the 0004 data migration, outbox queue behavior (incl. kind-routing + poison rule), highlight insert/bump/unarchive/409 semantics, the `textFragment` helper, note patch semantics (trim→NULL, never bumps `updated_at`) + by-url lookup/patch user scoping, pin semantics (m13: pin/unpin never bumps `updated_at`, archive↔pin mutual exclusion, feed-log exclusion + shelf ordering + search findability, live re-save keeps `pinned_at`), web-add semantics (`addBookmark`: hostname title autofill, bump+unarchive-only conflict, per-user dedupe), the web-add metadata fill (`enrichBookmarkMetadata`: never bumps `updated_at`, never overwrites an owned title — including against a mid-flight edit — fills `favicon_url` only when null, skips the fetch when nothing is missing, never throws; plus the favicon/title HTML parsing helpers in `firecrawl.ts`), the boundary-aware chunker (`chunkText`: limit never exceeded, separator fallback order, losslessness, the 4096-char TTS cap), article job semantics (`claimArticleRun`/`runArticleJob`: status machine, resume-skips-completed-steps, reset, stale-run re-claim, user scoping, and that NOTHING in the pipeline bumps `bookmarks.updated_at`), and TTS segmentation (per-request cap, ordered concatenation under out-of-order responses, all-or-nothing failure).
