...
17. Web-add metadata fill: `favicon_url` column; `scrapePageMetadata` (Firecrawl rawHtml → title + declared favicon) and `enrichBookmarkMetadata` (never bumps `updated_at`, never clobbers an owned title, never throws); `POST /api/bookmarks` waits on it (bounded, finishing in `after()`), `/share` fills in the background; composer pending state + stored-favicon rendering with the s2 fallback (§3, §5, §8, §9).
