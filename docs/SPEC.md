...
create table smultron.browse_events (
  ...
  kind            text not null,          -- 'nav' | 'tab_activated' | 'window_focus' | 'window_blur' | 'idle'
...
-   `unique (user_id, client_event_id)` on `browse_events` (idempotent batch inserts) and btree on `browse_events (user_id, occurred_at desc)` for the log view + retroactive analysis (m19)
...
-   **Attention tracking capture (m19)**: opt-in browse-event capture per §13. Manifest adds `idle`, `webNavigation`, and `history` permissions (`history` is held for the deferred backfill and adds no install warning beyond what broad `tabs` already triggered). Listeners (registered top-level per MV3, gated INSIDE the handler on the §13 toggle): `webNavigation.onCommitted` + `onHistoryStateUpdated` (main frame only) → `nav`; `tabs.onActivated` → `tab_activated`; `windows.onFocusChanged` → `window_focus`/`window_blur`; `idle.onStateChanged` (detection interval 60s) → `idle`. Events append to a storage-backed buffer (serialized via an in-worker promise-chain mutex; pure helper in `src/`, unit-tested) and drain into outbox `browse` entries of ≤500 (drain on: buffer ≥50 after an append, a dedicated 1-minute alarm, startup). Outbox `browse` entries POST to `/api/browse-events` and follow the HIGHLIGHT failure semantics (poison rule: definitive 4xx except 401 drops the entry and continues — telemetry must never wedge the queue ahead of bookmark syncs; 401/5xx/network halt). The popup owns the toggle (§13).
...
-   **Opt-in, and off means OFF.** Capture is gated on an explicit toggle (default disabled). Off = no events observed, buffered, or sent — zero capture, not zero notifications. Already-captured events still drain (they were captured while on).
...
Each event carries `clientEventId` (extension-minted UUID — the idempotency key) and `occurredAtMs` (client clock at capture; `webNavigation` events use the event's own `timeStamp`). Kind-specific fields:

| kind | required | optional | source |
| --- | --- | --- | --- |
| `nav` | `tabId`, `url` | `windowId`, `transition` | `webNavigation.onCommitted` + `onHistoryStateUpdated`, main frame (`frameId === 0`) only — `onHistoryStateUpdated` is what catches SPA navigations (YouTube, Twitter). No `title`: at commit time the tab still has the previous page's. |
| `tab_activated` | `tabId` | `windowId`, `url`, `title` | `tabs.onActivated`, enriched via `tabs.get` (url/title omitted if the lookup fails — the activation is still recorded). |
...
**Baseline events**: a dwell slicer needs to know what was active when capture started. On browser startup and on the toggle flipping to enabled, the extension emits a synthetic `tab_activated` for the active tab of the last-focused window (skipped when no Chrome window has focus). Service-worker death/revival mid-session needs no baseline — state didn't change while the worker was dead, and listeners re-fire.

### Capture pipeline (extension)

Listener → toggle gate → buffer append (`chrome.storage.local`, appends serialized through an in-worker promise-chain mutex) → drain into outbox `browse` entries (≤500 events each; triggers: buffer ≥50 after an append, a 1-minute alarm, startup) → `POST /api/browse-events` with the highlight-style poison rule (§6). At-least-once delivery end to end; the server dedupes on `(user_id, client_event_id)`.

The opt-in toggle lives in `chrome.storage.local` under its own key (`attention`, `{enabled: boolean}`; missing = disabled — NOT in the options-page config object, which is rewritten wholesale on save). The popup renders it in every paired state (it's a global setting, not per-tab): a switch labeled for attention tracking, plus a session-grade slot that shows a placeholder ("collecting data") until RED-92/93 detectors exist. The background reacts to the key via `storage.onChanged` (baseline emission on enable); the popup itself never captures anything.

### Server semantics

`applyBrowseEvents` (web `lib/browseEvents.ts`): normalize `url` → `url_normalized` (§4, same single implementation), insert append-only, `ON CONFLICT (user_id, client_event_id) DO NOTHING`, return `{inserted, deduped}`. Batches are validated strictly (Zod, per-kind field requirements above, unknown fields rejected) — a malformed batch is a 400 and the outbox poison rule drops it; the extension's own unit tests are what keep batches well-formed. `listBrowseEvents` powers the log view (§8, §9). Rows are never updated or deleted (append-only telemetry; retention is a future decision).
