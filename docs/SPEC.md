...
-   **Add composer (m11 — from the approved Claude Design mock `Smultron Feed - Add Bookmark.dc.html`)**: an accent-solid `+ Add` button at the toolbar's right edge toggles an inline composer bar pinned above the log (`+` glyph, autofocused URL input in mono, Save button, `esc` closer). Enter/Save: the client prepends `https://` to scheme-less input, validates (parseable, dotted hostname — inline mono "not a valid URL" error otherwise), then renders the bookmark IMMEDIATELY, before `POST /api/bookmarks` is in flight (m18 optimistic add — this replaced m17's blocking composer AND m18's first cut, which still waited on the POST plus a full revalidation before anything rendered): the composer closes at once, the view switches to the live feed (search/tag filters are kept), and an optimistic temp row (negative client-side id; the same hostname title the server autofills per §5; enriching shimmer already on) lands at the top of the log, flashing (`rowflash` keyframe fading from `--log-facet-active`), auto-expanded, with the panel's add-tag input focused. Invalid input (inline mono "not a valid URL") still keeps the composer open. The POST reconciles the row when it returns: the server's row replaces the temp row and every id-keyed state (expanded / flash / enriching / tag-input focus) follows the real id — a PATCH issued against a temp id first awaits the pending POST's real id; a resurfaced duplicate that is PINNED instead leaves the log for the shelf (m13: pinned rows are excluded from the log); a failed POST rolls the temp row back and reopens the composer with the draft preserved and an inline error. Overlay rows are released once a page-1 response carries their id, and dropped on entering the archived view. A duplicate URL resurfaces the existing row (bump + unarchive, §5) with the same flash+expand. Esc or `esc` closes and clears. **Enriching state (m18)**: a freshly added row renders its title and favicon as a loading shimmer (colors from `--log-*` vars) and suppresses the note preview while the §5 fill runs — from the moment the optimistic row renders; a resurfaced duplicate stops shimmering as soon as the POST identifies it (`created: false` — its metadata already exists, no fill is coming). The client snapshots the row's `{title, faviconUrl, note}` (re-snapshotted from the POST's row at reconcile) and clears the state when a poll delivers a row where ANY of the three differs from the snapshot, or after a 30s deadline (a failed fill leaves the hostname title — the shimmer must never outlive the fill's own timeout). While enriching, the client revalidates on a short interval (~2s, page-1 key only) so the fill lands visibly fast; the row stays fully interactive (expand, tag, archive), and a USER edit of the row's title or note ends the shimmer immediately — they own the field now (§5's mid-flight guard already protects the write side).
...
18. Instant web add: `POST /api/bookmarks` returns immediately (m17's bounded wait removed — the fill always completes in `after()`); the composer closes and an optimistic temp row (shimmer already on) renders before the POST is in flight, reconciled to the server's row on return (temp-id swap, rollback + reopened composer on failure); client-owned enriching shimmer on the new row's title/favicon with snapshot-diff + 30s deadline clearing and short-interval revalidation (§5, §8, §9).
