...
-   **Service worker**:
    -   `onCreated` listener → enqueue `{mode:'live', bookmark}` in outbox → flush. The live entry also carries `faviconUrl` when a tab is open on the URL (`chrome.tabs.query({url})`, first match's `favIconUrl`; pure helper `src/favicon.ts`, unit-tested) — see §5. Backfill entries never carry it.
...
-   **New tab page (m20)**: `entrypoints/newtab/` replaces Chrome's new tab via `chrome_url_overrides.newtab` (wired by WXT's reserved `newtab` entrypoint name — no manifest edit). **Unconditional, and deliberately NOT toggleable** (approved 2026-08-30): Chrome exposes no runtime switch for a new-tab override, so a "disabled" state could only render a blank extension page — which is not Chrome's default new tab either. The alternative (no override; the background redirects new tabs while a toggle is on) was rejected for the visible flash and the lost empty-omnibox behavior. Installing the extension therefore takes over the new tab, and Chrome says so at install time.
    ...
    -   Content, top to bottom: a **PINNED shelf** (the response's `pinned` array — favicon + host + 2-line-clamped title cards, click opens the bookmark), then a **RECENT log** (the live feed's first page, dense rows: `updated_at` time, favicon, host, title, tag chips; since m22 it includes pinned rows — a server-side change it inherits for free — marked with the same accent `★` the site's log rows carry). Both come from ONE `GET /api/bookmarks` with the pairing token (§8) — the page never queries the DB and never uses the outbox (it is a read surface with a user present, like the popup: direct fetch, truthful outcomes).
