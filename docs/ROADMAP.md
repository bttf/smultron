...
- **Hardening the outbox contract** (applies beyond this feature, but this feature depends on it): flush must process the queue strictly FIFO, sequentially — await each entry's terminal resolution (2xx, or requeue on failure) before advancing to the next. Given that, and given step 3/4 append order above, a highlight can never reach the server before its bookmark's insert has either succeeded or is still queued ahead of it. No server-side "wait for the bookmark to exist" logic is needed to close the race.
